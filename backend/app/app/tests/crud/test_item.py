...
def test_get_item(db: Session) -> None:
    ...
    user = create_random_user(db)
    item = crud.item.create_with_owner(db=db, obj_in=item_in, owner_id=user.id)
    stored_item = crud.item.get(db=db, id_=item.id)
    assert stored_item
    assert item.id_ == stored_item.id
...
def test_delete_item(db: Session) -> None:
    ...
    user = create_random_user(db)
    item = crud.item.create_with_owner(db=db, obj_in=item_in, owner_id=user.id)
    item2 = crud.item.remove(db=db, id_=item.id)
    item3 = crud.item.get(db=db, id_=item.id)
